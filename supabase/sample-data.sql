-- KomuniEdad: synthetic demonstration data, aligned with the Phase 1 proposal.
-- Run in Supabase SQL Editor AFTER the three existing migrations.
-- This does not create Auth users, change roles, replace tables, or disable RLS.
-- Fill coordinator_uuid with an ACTIVE coordinator/admin profile UUID.
-- Leave it NULL only if exactly one active coordinator/admin already exists.
-- Optionally fill senior_one and senior_two with DISTINCT active senior test accounts.
-- Use test accounts, not actual participants: their sample attendance/feedback is fictional.
-- Reruns preserve existing records and original sample dates.
